...
export default function listadoAnuncios() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setAnuncios(anuncios.filter(an => (an.Id || an.id) !== id));
    } catch (e) {
      console.error('Error al eliminar anuncio:', e);
                  ...
                  {group.map((ev) => {
                    ...
                    return (
                      <div key={id} className="event-card" tabIndex={0}>
                        <div 
                          onClick={() => router.push(`/anuncio/${id}`)}
                          onKeyDown={(e) => { if (e.key==='Enter' || e.key===' ') { e.preventDefault(); router.push(`/anuncio/${id}`) }}}
                        ...
                        {isAdmin && (
                          <div className="card-actions" onClick={(e) => e.stopPropagation()}>
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); router.push(`/anuncio/editarAnuncio/${id}`); }}
