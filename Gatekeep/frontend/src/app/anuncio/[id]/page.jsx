...
export default function AnuncioDetalle() {
  ...
  useEffect(() => {
    ...
    const fetchAnuncio = async () => {
      try {
        const resp = await AnuncioService.getAnuncio(id)
        setAnuncio(resp.data)
      } catch (e) {
            ...
            {loading ? (
              ...
              <article className="card">
                <header className="card-header">
                  <h1 className="title">{anuncio.Nombre || anuncio.nombre || 'Anuncio'}</h1>
                  <span className={`badge ${((anuncio.Activo ?? anuncio.activo) ? 'ok' : 'off')}`}>
                    {(anuncio.Activo ?? anuncio.activo) ? 'Activo' : 'Inactivo'}
                  </span>
                </header>
                ...
                <div className="meta">
                  <div className="meta-item">
                    ...
                    <span className="meta-value">{new Date(anuncio.Fecha || anuncio.fecha).toLocaleDateString('es-ES', { year:'numeric', month:'long', day:'numeric'})}</span>
                  </div>
                </div>

                {/* Espacio para más campos futuros */}
                <div className="body">
                  <p className="hint">No hay más información para este anuncio.</p>
                </div>
              </article>
            ) : null}
          </div>
