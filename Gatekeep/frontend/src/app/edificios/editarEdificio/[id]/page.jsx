...
export default function EditarEdificioPage() {
	...
	const edificioId = parseInt(params.id, 10);
	...
	useEffect(() => {
		...
		const fetchEdificio = async () => {
			try {
				const response = await EdificioService.getEdificioById(edificioId);
				const edificio = response.data;
				setNombre(edificio.Nombre || edificio.nombre || '');
				setDescripcion(edificio.Descripcion || edificio.descripcion || '');
				setUbicacion(edificio.Ubicacion || edificio.ubicacion || '');
	...
	const handleSubmit = async () => {
		...
		console.log('Enviando PUT a edificio ID:', edificioId);
		console.log('Payload:', payload);
		setSubmitting(true);
		try {
			const response = await EdificioService.updateEdificio(edificioId, payload);
			...
			console.error('Error actualizando edificio:', e);
			console.error('Error completo:', JSON.stringify(e, null, 2));
			setError(e.response?.data?.error || e.response?.data?.message || e.message || 'Error al actualizar el edificio');
		} finally {
			setSubmitting(false);
		}
	};
		...
		<div className="header-root">
			<div className="header-hero">
				...
				<div className="header-middle-bar">
					<form className="text-card" onSubmit={(e) => { e.preventDefault(); if (!submitting) handleSubmit(); }} aria-label="Formulario editar edificio">
					<div style={{alignItems: 'center', width: '100%'}}>
						...
						{success && (
							<div style={{ color: '#e9ffe9', background:'#1e7e3a', borderRadius:12, padding:'8px 14px', margin:'10px 1vw', width:'calc(100% - 2vw)', fontSize:'0.85rem' }}>Edificio actualizado. Redirigiendo...</div>
						)}
						<div className='button-container'>
							<button type="submit" disabled={submitting} style={{ opacity: submitting ? 0.6 : 1, cursor: submitting ? 'not-allowed' : 'pointer' }}>
								{submitting ? 'Actualizando...' : 'Actualizar Edificio'}
							</button>
						</div>
					</form>
			</div>
