...
export default function listadoReglasAcceso() {
  ...
  // Filter reglas based on search input (case-insensitive) and optional date range
  const filteredReglas = useMemo(() => {
    const q = searchInput.trim().toLowerCase();
    return reglas.filter((regla) => {
      // Filter by query on EspacioId
      if (q) {
        const espacioId = (regla.EspacioId ?? regla.espacioId ?? '').toString();
        if (!espacioId.includes(q)) return false;
      }

      // Filter by date range if provided
      if (dateFrom) {
        const vigenciaApertura = regla.VigenciaApertura ? new Date(regla.VigenciaApertura).toISOString().split('T')[0] : '';
        if (!vigenciaApertura || vigenciaApertura < dateFrom) return false;
      }
      if (dateTo) {
        const vigenciaCierre = regla.VigenciaCierre ? new Date(regla.VigenciaCierre).toISOString().split('T')[0] : '';
        if (!vigenciaCierre || vigenciaCierre > dateTo) return false;
      }

      return true;
    });
  }, [reglas, searchInput, dateFrom, dateTo]);

  // Group filtered reglas in chunks of 4 so visual grouping of 4 is preserved
  const groupedReglas = [];
  for (let i = 0; i < filteredReglas.length; i += 4) {
    groupedReglas.push(filteredReglas.slice(i, i + 4));
  }
        ...
        <div className="container">
              ...
              <div className="field">
                ...
                <form className="search-bar" onSubmit={handleSearchSubmit}>
                  <input
                    ...
                    type="text"
                    placeholder="Buscar por espacio ID..."
          ...
          <div className="events-grid">
            {loading ? (
              ...
              groupedReglas.map((group, gi) => (
                <div className="event-group" key={`group-${gi}`}>
                  {group.map((regla) => {
                    const id = regla.Id ?? regla.id;
                    const espacioId = regla.EspacioId ?? regla.espacioId ?? 'N/A';
