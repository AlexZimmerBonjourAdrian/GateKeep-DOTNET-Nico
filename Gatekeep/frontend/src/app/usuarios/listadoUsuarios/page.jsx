...
export default function ListadoUsuarios() {
	...
	const filteredUsuarios = useMemo(() => {
		...
		return usuarios.filter(u => {
			// Filtrar por rol
			const rol = u.Rol || u.rol || u.TipoUsuario || u.tipoUsuario || '';
			if (rolFilter !== 'Todos' && rol !== rolFilter) return false;
			...
			if (q) {
				...
				if (!nombre.includes(q) && !apellido.includes(q) && !email.includes(q) && !nombreCompleto.includes(q)) {
					return false;
				}
			}
			return true;
	...
	const grouped = [];
	for (let i = 0; i < filteredUsuarios.length; i += 4) {
		grouped.push(filteredUsuarios.slice(i, i + 4));
	}
	...
	const handleDelete = async (id) => {
		...
		try {
			...
			setUsuarios(usuarios.filter(u => (u.Id || u.id) !== id));
		} catch (e) {
	...
	return (
		<div className="container-nothing">
				...
				<div className="container-header">
					...
					<div className="filtros-container">
						...
						<div className="field" style={{ minWidth: '160px' }}>
							<label className="field-label" htmlFor="rol-filter">Filtrar por rol</label>
							<select
								id="rol-filter"
						...
						<div className="actions-inline">
							<button
								...
								onClick={() => router.push('/usuarios/crearUsuario')}
					...
					{loading ? (
						...
						grouped.map((group, gi) => (
							...
							{group.map((u) => {
								...
								const telefono = u.Telefono || u.telefono || 'N/A';
								const rol = u.Rol || u.rol || u.TipoUsuario || u.tipoUsuario || 'N/A';
									...
									>
										...
										<div className="card-actions" onClick={(e) => e.stopPropagation()}>
											<button
												...
												onClick={(e) => { e.stopPropagation(); router.push(`/usuarios/editarUsuario/${id}`); }}
												aria-label={`Editar ${nombre} ${apellido}`}
											>
												✏️ Editar
											</button>
											<button
												...
												aria-label={`Eliminar ${nombre} ${apellido}`}
											>
												🗑️ Eliminar
											</button>
										</div>
			...
			<style jsx>{`
				...
				.rol-select { padding: 8px 12px; background: #f8fafc; border: 1px solid #e5e7eb; border-radius: 20px; font-size: 0.95rem; color: #111827; cursor: pointer; outline: none; }
				.rol-select:focus { border-color: #f37426; }
