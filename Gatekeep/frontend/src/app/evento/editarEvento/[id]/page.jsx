...
export default function EditarEventoPage() {
	...
	useEffect(() => {
		if (!eventoId || isNaN(eventoId)) return;
		const fetchEvento = async () => {
			try {
				const response = await EventoService.getEvento(eventoId);
				const evento = response.data;
				setNombre(evento.Nombre || evento.nombre || '');
				const fechaValue = evento.Fecha || evento.fecha;
				if (fechaValue) {
				...
				setResultado(evento.Resultado || evento.resultado || '');
				setPuntoControl(evento.PuntoControl || evento.puntoControl || '');
			} catch (e) {
	...
	const validate = () => {
		...
		if (selected < today) return 'La fecha del evento no puede ser anterior a hoy';
		return null;
	};
	...
	const handleSubmit = async (e) => {
		...
		console.log('Enviando PUT a evento ID:', eventoId);
		console.log('Payload:', payload);
		setSubmitting(true);
		try {
			const response = await EventoService.updateEvento(eventoId, payload);
			console.log('Respuesta recibida:', response);
			if (response.status >= 200 && response.status < 300) {
				setSuccess(true);
				setTimeout(() => router.push('/evento/listadoEventos'), 900);
			} else {
			...
			setError(e.response?.data?.error || e.response?.data?.message || e.message || 'Error al actualizar el evento');
		} finally {
			setSubmitting(false);
		}
	};
		...
		<div className="header-root">
			<div className="header-hero">
				...
				<div className="header-middle-bar">
					<form className="text-card" onSubmit={handleSubmit} aria-label="Formulario editar evento">
						...
						<div className='input-container'>
							<div className='w-full'>
								<span>Nombre *</span>
								<input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Nombre del Evento" />
							</div>
							<div className='w-full'>
								<span>Fecha *</span>
								<input type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} placeholder="Fecha del Evento" />
							</div>
							<div className='w-full'>
								<span>Resultado</span>
								<input type="text" value={resultado} onChange={(e) => setResultado(e.target.value)} placeholder="Resultado del Evento" />
							</div>
						<div className='w-full'>
							<span>Punto de Control</span>
							<input type="text" value={puntoControl} onChange={(e) => setPuntoControl(e.target.value)} placeholder="Punto de Control del Evento" />
						</div>
					</div>
						{error && (
						...
						{success && (
							<div style={{ color: '#e9ffe9', background:'#1e7e3a', borderRadius:12, padding:'8px 14px', margin:'10px 1vw', width:'calc(100% - 2vw)', fontSize:'0.85rem' }}>Evento actualizado. Redirigiendo...</div>
						)}
