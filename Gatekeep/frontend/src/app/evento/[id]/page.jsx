...
export default function EventoDetalle() {
  ...
  useEffect(() => {
    ...
    const fetchEvento = async () => {
      try {
        const resp = await EventoService.getEvento(id)
        setEvento(resp.data)
      } catch (e) {
            ...
            {loading ? (
              ...
              <article className="card">
                <header className="card-header">
                  <h1 className="title">{evento.Nombre || evento.nombre || 'Evento'}</h1>
                  <span className={`badge ${((evento.Activo ?? evento.activo) ? 'ok' : 'off')}`}>
                    {(evento.Activo ?? evento.activo) ? 'Activo' : 'Inactivo'}
                  </span>
                </header>
                ...
                <div className="meta">
                  <div className="meta-item">
                    ...
                    <span className="meta-value">{new Date(evento.Fecha || evento.fecha).toLocaleDateString('es-ES', { year:'numeric', month:'long', day:'numeric'})}</span>
                  </div>
                  <div className="meta-item">
                    <span className="meta-label">Punto de control</span>
                    <span className="meta-value">{evento.PuntoControl || evento.puntoControl}</span>
                  </div>
                  <div className="meta-item">
                    <span className="meta-label">Resultado</span>
                    <span className="meta-value">{evento.Resultado || evento.resultado}</span>
                  </div>
                </div>

                <div className="body">
                  <p className="hint">No hay más información para este evento.</p>
                </div>
              </article>
            ) : null}
          </div>
