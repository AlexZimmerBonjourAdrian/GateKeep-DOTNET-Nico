...
export default function listadoBeneficios() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setBeneficios(beneficios.filter(b => (b.Id || b.id) !== id));
    } catch (e) {
  ...
  return (
    <div className="container-nothing">
        ...
        <style jsx>{`
          ...
          .history-button{ background:#231F20; color:#fff; border:none; padding:8px 16px; border-radius:20px; cursor:pointer; font-size:0.85rem; font-weight:600; letter-spacing:0.3px; box-shadow:0 2px 6px rgba(0,0,0,0.15); transition:background 0.15s ease, transform 0.15s ease; display:flex; align-items:center; }
          .history-button:hover{ background:#3d3739; transform:translateY(-2px); }
          .history-button:active{ transform:translateY(0); }
          .history-button:focus-visible{ outline:2px solid rgba(37,99,235,0.4); outline-offset:2px; }
