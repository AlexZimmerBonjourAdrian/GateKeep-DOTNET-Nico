...
# Generar password aleatorio para RabbitMQ
...
# Versión del secret con RabbitMQ password
...
locals {
  ...
  rabbitmq_password_secret_string = var.manage_secret_versions ? aws_secretsmanager_secret_version.rabbitmq_password[0].secret_string : try(data.aws_secretsmanager_secret_version.rabbitmq_password[0].secret_string, "")
}
